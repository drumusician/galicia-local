defmodule GaliciaLocalWeb.ClaimBusinessLive do
  ...
  def mount(%{"id" => id}, _session, socket) do
    case Business.get_by_id(id) do
      {:ok, business} ->
        ...
        {:ok,
         socket
         |> assign(:page_title, "Claim #{business.name}")
  ...
  def handle_event("submit_claim", _params, socket) do
    ...
    case BusinessClaim
      ...
      {:error, _changeset} ->
        {:noreply, put_flash(socket, :error, "Could not submit claim. You may have already claimed this business.")}
    end
  end
      ...
      <div class="container mx-auto max-w-2xl px-4 py-8">
        <nav class="text-sm breadcrumbs mb-6">
          <ul>
            ...
            <li class="text-base-content/60">Claim</li>
          </ul>
        </nav>
            ...
            <%= cond do %>
              <% @business.owner_id != nil -> %>
                <div class="alert alert-warning">
                  ...
                  <span>This business has already been claimed by its owner.</span>
                </div>
              ...
              <% @existing_claim != nil and not @submitted -> %>
                <div class="alert alert-info">
                  ...
                  <div>
                    <p class="font-medium">You already submitted a claim for this business.</p>
                    <p class="text-sm">Status: <span class="badge badge-sm">{@existing_claim.status}</span></p>
                  </div>
                </div>
                <div class="card-actions mt-4">
                  <.link navigate={~p"/businesses/#{@business.id}"} class="btn btn-ghost">
              ...
              <% @submitted -> %>
                <div class="alert alert-success">
                  ...
                  <div>
                    <p class="font-medium">Claim submitted!</p>
                    <p class="text-sm">We'll review your claim and get back to you. Once approved, you'll be able to manage your business listing.</p>
                  </div>
                </div>
                <div class="card-actions mt-4">
              ...
              <% true -> %>
                <p class="text-base-content/70 mb-6">
                  Are you the owner of this business? Submit a claim and our team will verify your ownership.
                  Once approved, you'll be able to update your business information, add photos, and more.
                </p>

                <form phx-submit="submit_claim" class="space-y-4">
                  <div class="form-control">
                    <label class="label">
                      <span class="label-text font-medium">Why are you the owner?</span>
                    </label>
                    <textarea
                      name="message"
                      phx-change="update_message"
                      placeholder="Tell us how you're connected to this business (e.g. 'I am the founder', 'I manage this location')"
                      class="textarea textarea-bordered w-full h-32"
                      value={@message}
                    >{@message}</textarea>
                    <label class="label">
                      <span class="label-text-alt text-base-content/50">Optional but helps us verify faster</span>
                    </label>
                  </div>
